...
<template>
    <SeoHead :title="title" :description="description" :og-image="heroImage?.url" :preload-image="heroImage?.url" />
    ...
    <section class="hero" :class="{ 'hero--split': heroImage }">
        <div class="wrap hero__inner">
            ...
            <div v-if="heroImage" class="hero__media">
                <img :src="heroImage.url" :width="heroImage.width" :height="heroImage.height"
                     alt="Personna" fetchpriority="high" decoding="async" />
            </div>
        </div>
    </section>

    <section class="wrap catalogue">
        <h2 class="visually-hidden">{{ $t('catalogue.all') }}</h2>
